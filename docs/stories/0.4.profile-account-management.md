...
**Email Change:**
- [ ] Click Change Email → dialog opens
- [ ] Submit with invalid email → shows validation error
- [ ] Submit with wrong password → shows error toast
- [ ] Submit with correct password → shows success toast, verification email sent
- [ ] Click verification link in email → email updated, redirected to login
- [ ] Re-login with new email → works correctly
- [ ] Pending email change expires after 1 hour → shows error on verify
...
- [ ] Deletion failure (e.g., S3 delete fails) → shows error, no data deleted (rollback)

**Regression Testing:**
- [ ] Edit name → still works
