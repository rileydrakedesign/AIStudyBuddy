...
- Visual effect: `"H" → "He" → "Hel" → "Hell" → "Hello"`
...
**Complexity**: Medium (3-layer change: Python → Node → Frontend)
...
- [x] Performance improvement expected (first token: 3-5s → 1-2s)
...
pythonStream.data.on('data', (chunk) => {
  // Parse SSE → emit WebSocket events
  io.to(userRoom).emit('chat-stream-token', { sessionId, token });
});

// ⚠️ FIX (timing bug):
// Move: chatSession.save() → inside stream.on('end')
// Move: chatRequestCount++ → inside stream.on('end')

// ✅ PRESERVE (200+ lines unchanged):
- Auth check
- Session creation
- User message push
...
- **Critical Conflict**: SSE vs WebSocket architecture mismatch → Resolved by using WebSocket per CLAUDE.md
- **Missing Integration**: Node backend WebSocket emission → Added complete specification
- **Incorrect Pattern**: Python callback/yield pattern → Fixed with async queue pattern
- **Incomplete Redis**: Stream state management → Added key patterns and cleanup
- **Missing Auth**: SSE authentication issues → Resolved by using authenticated WebSocket
- **Missing Recovery**: Stream interruption handling → Added comprehensive error recovery
- **Missing Config**: Environment variables → Added to all 3 services
- **Memory Leaks**: Listener cleanup → Added useEffect cleanup specification
- **Free Tier Accounting**: Timing clarified → Only count successful streams
- **Monitoring**: Observability → Added streaming metrics
