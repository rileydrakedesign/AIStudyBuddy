...
export const generateChatCompletion = async (req, res, next) => {
    ...
    try {
        ...
        req.log.debug({ pythonApiUrl }, "Selected Python API URL");
        ...
        try {
            req.log.info({ streamEndpoint, userId: userId.toString() }, "Starting Python streaming request");
                ...
                events.forEach((eventBlock) => {
                    try {
                        ...
                        if (event.type === 'token') {
                            tokenCount++;
                            if (tokenCount === 1) {
                                req.log.info("Received first token from Python");
                            }
                            ...
                            req.log.info({ tokenCount }, "Received done event from Python");
                            ...
                            // Python error
                            req.log.error({ message: event.message }, "Received error event from Python");
                            streamError = event.message;
                        }
                        else if (event.type === 'keepalive') {
                            keepaliveCount++;
                            if (keepaliveCount % 10 === 1) {
                                req.log.debug({ keepaliveCount }, "Received keepalive from Python");
                            }
            ...
            pythonStream.data.on('end', async () => {
                req.log.info({ streamError, hasCitations: !!citations }, "Python stream ended");
                if (streamError) {
                    // Stream failed - emit error and don't save
                    req.log.error({ streamError }, "Stream failed with error");
            ...
            pythonStream.data.on('error', (error) => {
                req.log.error({ err: error }, "Python stream error");
                io.to(userRoom).emit('chat-stream-error', {
                    sessionId: chatSession._id.toString(),
                    error: error.message
                });
            ...
            req.log.error({ err: streamInitError }, "Failed to initiate stream");
