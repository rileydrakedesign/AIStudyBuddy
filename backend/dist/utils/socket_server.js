...
-------------------------------------------------------------------*/
const log = (...args) => console.log("\x1b[35m[WS‑auth]\x1b[0m", ...args); // magenta tag
/*-------------------------------------------------------------------
...
export const initializeWebSocket = (server /* http.Server */) => {
    ...
    io.use((socket, next) => {
        ...
        if (token.startsWith("s:")) {
            ...
            const unsigned = signature.unsign(token.slice(2), process.env.COOKIE_SECRET || process.env.JWT_SECRET);
    ...
    io.on("connection", (socket) => {
        ...
        socket.on("disconnect", (reason) => log(`⚠️  socket ${socket.id} disconnected →`, reason));
    });
    /*------------- Mongo change‑stream listener (unchanged) -----------------*/
    Document.watch([{ $match: { "updateDescription.updatedFields.isProcessing": false } }], { fullDocument: "updateLookup" }).on("change", (change) => {
